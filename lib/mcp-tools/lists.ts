...
async function handleListLists(user: User) {
  try {
...
async function handleCreateList(user: User, { name }: { name: string }) {
  try {
...
async function handleGetList(user: User, { listId, month, year }: { listId: string; month?: number; year?: number }) {
  try {
...
async function handleRenameList(user: User, { listId, name }: { listId: string; name: string }) {
  try {
...
async function handleDeleteList(user: User, { listId }: { listId: string }) {
  try {
...
export function registerListTools(server: McpServer, user: User) {
  server.tool("list_lists", "Devuelve todas las listas del usuario.", {}, () => handleListLists(user));

  server.tool("create_list", "Crea una nueva lista.", { name: z.string().min(1).max(100) }, (p) =>
    handleCreateList(user, p)
  );
